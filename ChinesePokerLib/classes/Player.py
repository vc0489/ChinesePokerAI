...
class ChinesePokerPlayer:
  ...
  def _add_id(self):
    ChinesePokerPlayer._players_created += 1
    self.id = ChinesePokerPlayer._players_created
...
class HumanChinesePokerPlayer(ChinesePokerPlayer):
  def __init__(self):
    super().__init__()
    return

class ComputerChinesePokerPlayer(ChinesePokerPlayer):
  def __init__(
    self, 
    strategy,
  ...
  def play_hand(self):
    ...
    # VC TODO: add var checking (see Hand class)
